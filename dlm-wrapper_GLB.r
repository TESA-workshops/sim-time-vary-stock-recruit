...
fitDLM <- function(data = bt,
  ...
  dlmPars=3
  if(alpha_vary==TRUE & beta_vary==FALSE){dlmPars=4; nstate=length(data$rec)}
  if(alpha_vary==FALSE & beta_vary==TRUE){dlmPars=4; nstate=length(data$rec)}
  if(alpha_vary==TRUE & beta_vary==TRUE){dlmPars=5; nstate=2*length(data$rec)}
  ...
  dlm_out<-dlmMLE(y=lnRS, build=build_mod, parm=c(-.1,-.1,-.1), method="Nelder-Mead",hessian=TRUE)

  # 5. log-likelihood
  lls <- dlm_out$value

  H <- dlm_out$hessian
  if(alpha_vary==TRUE & beta_vary==FALSE){H <- H[1:2,1:2]} # evolution (process error) variances
  if(alpha_vary==FALSE & beta_vary==TRUE){H <- H[c(1,3),c(1,3)]}
  ...
  AIC   <- 2*lls + 2*dlmPars
  BICd  <- 2*lls + dlmPars*log(length(data$rec)) - dlmPars*log(2*pi) - log(abs(det(solve(H)))) 
  
  AICc_s	<- 2*lls + 2*(dlmPars+nstate) + (2*dlmPars*(dlmPars+1)/(length(data$rec)-dlmPars-1))
  BIC_s   <- 2*lls + (dlmPars+nstate)*log(length(data$rec))
  AIC_s   <- 2*lls + 2*(dlmPars+nstate)
  BICd_s  <- 2*lls + (dlmPars+nstate)*log(length(data$rec)) - (dlmPars+nstate)*log(2*pi) - log(abs(det(solve(H)))) 
